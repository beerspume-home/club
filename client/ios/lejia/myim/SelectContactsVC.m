...
@interface SelectContactsVC ()<UITableViewDelegate,UITableViewDataSource>{
    Person* _person;
...
-(void)reloadView{
    //标题栏
    UIView* headView=[[HeaderView alloc]initWithTitle:@"添加成员"
    ...
    _tableView.top=searchView.bottom;
...
-(void)ok{
    if(_selectedPerson.count==0){
        [Utility showError:@"请选择成员" type:ErrorType_Business];
        return;
    }
    [self gotoBackWithParamaters:@{
                                   PAGE_PARAM_PEOPLE:_selectedPerson,
                                   }];
}
