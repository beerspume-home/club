...
#import "CustomerServiceInfoVC.h"

@interface CustomerServiceInfoVC (){
    Person* _person;
...
@implementation CustomerServiceInfoVC
...
-(void)ok{
    if(_choicedSchool==nil){
        ...
        [Remote createCustomerService:_person.id schoolid:_choicedSchool.id callback:^(StorageCallbackData *callback_data) {
            if(callback_data.code==0){
                CustomerService* obj=callback_data.data;
                [self gotoBackToViewController:[MyInfoVC class] paramaters:@{
                                                                             PAGE_PARAM_CUSTOMERSERVICE:obj,
                                                                             }
                 ];
            }else{
