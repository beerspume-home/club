...
#import "TATeacherRecordListVC.h"
#import "TATeacherRecordCell.h"

#pragma mark TeacherAppointmentDetailView
@interface TeacherAppointmentDetailView : UIControl
@end
@protocol TeacherAppointmentDetailViewDelegate <NSObject>
-(void)appointmentDetailView:(TeacherAppointmentDetailView*)appointmentDetailView cancelAppointment:(id)data;
@end

@interface TeacherAppointmentDetailView(){
    UIView* _rootView;
...
@property (nonatomic,retain) id<TeacherAppointmentDetailViewDelegate> delegate;

@end
@implementation TeacherAppointmentDetailView
...
-(instancetype)init{
    TeacherAppointmentDetailView* ret=[super init];
...
-(void)layoutSubviews{
    ...
    _nameLabel.text=[NSString stringWithFormat:@"学员:%@",_appointment.teacher.person.name];
    ...
    NSString* remarkText=[NSString stringWithFormat:@"备注:\n%@",_course.remark];
...
@interface TATeacherRecordListVC ()<UITableViewDataSource,UITableViewDelegate,TeacherAppointmentDetailViewDelegate,TATeacherRecordCellDelegate>{
    ...
    TeacherAppointmentDetailView* _appointmentDetailView;
    
}

@end

@implementation TATeacherRecordListVC
...
-(void)reloadRemoteData{
    if(!_loading){
        _loading=true;
        [Remote teacherAppointmentList:_teaherid showexpired:_showExpired start:_start offset:_offset callback:^(StorageCallbackData *callback_data) {
    ...
    if(_showExpired){
        if(![appointment isDeleted]){
            [self gotoPageWithClass:[TAAppointmentInfoVC class] parameters:@{
                                                                               ...
                                                                               PAGE_PARAM_TEACHER_ID:_teaherid,
                                                                               PAGE_PARAM_INDEX:[NSNumber numberWithInt:index],
                                                                               }];
        }
    }else{
        if(_appointmentDetailView==nil){
            _appointmentDetailView=[[TeacherAppointmentDetailView alloc]init];
...
-(void)sendMessage:(TeacherAppointmentDetailView *)appointmentDetailView student:(Student*)student{
    
    JEAppointmentMessage* rcMessage=[[JEAppointmentMessage alloc]init];
    rcMessage.teacherid=_teaherid;
    rcMessage.studentid=student.id;
    ...
    [[RCIMClient sharedRCIMClient]sendMessage:ConversationType_PRIVATE targetId:student.person.id content:rcMessage pushContent:@"" pushData:nil success:^(long messageId) {
...
-(void)appointmentDetailView:(TeacherAppointmentDetailView *)appointmentDetailView cancelAppointment:(id)data{
    ...
    [Remote cancelAppointment:appointmentDetailView.appointment.student.id date:appointmentDetailView.appointment.date courseid:appointmentDetailView.appointment.course.id callback:^(StorageCallbackData *callback_data) {
        if(callback_data.code==0){
            appointmentDetailView.hidden=true;
            appointmentDetailView.appointment.deleted=@"1";
            [_tableView reloadData];
            [self sendMessage:appointmentDetailView student:appointmentDetailView.appointment.student];
        }else{
            [Utility showError:callback_data.message];
        }
