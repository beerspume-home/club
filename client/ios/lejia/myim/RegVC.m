...
-(void)reloadView{
    ...
    _phoneTextField.size=CGSizeMake(_phoneTextField.superview.width-60, HEIGHT_BUTTON);
...
-(void)doReg{
    ...
    [Remote regWithPhone:phone smscode:smscode password:password callback:^(StorageCallbackData *callback_data) {
        if(callback_data.code==0){
            Person* loginInfo=callback_data.data;
            [Storage setLoginInfo:loginInfo];
            [Utility connectRongCloud];
            [self gotoPageWithClass:[MainVC class]];
        }else{
            
            [Utility showError:[NSString stringWithFormat:@"注册失败:%@",callback_data.message] type:ErrorType_Network];
        }
